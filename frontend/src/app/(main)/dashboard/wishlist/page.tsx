...
const wishlistItems = [
  { id: "1", title: "Lut Desert Adventure", type: "NATURE", duration: "4 Days", price: 680, rating: 4.5 },
  { id: "2", title: "Masuleh & Kandovan Villages", type: "VILLAGE", duration: "5 Days", price: 520, rating: 4.7 },
  { id: "3", title: "Caspian Sea Nature Tour", type: "NATURE", duration: "3 Days", price: 290, rating: 4.4 },
];
        ...
        {wishlistItems.map((item) => (
          <div key={item.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden group">
            <div className="h-48 bg-gradient-to-br from-emerald-400 to-teal-500 relative">
              <div className="absolute inset-0 flex items-center justify-center text-white text-4xl font-bold opacity-30">{item.title.charAt(0)}</div>
              <button className="absolute top-3 right-3 w-8 h-8 bg-white/90 rounded-full flex items-center justify-center text-red-500 hover:bg-white hover:scale-110 transition-all">
              ...
              <span className="absolute bottom-3 left-3 px-2 py-1 bg-white/90 text-xs font-medium text-gray-700 rounded">{typeLabels[item.type] || item.type}</span>
            </div>
            <div className="p-4">
