...
export default function AdminPastToursPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      else alert(data.error || t.admin.uploadFailed);
    } catch { alert(t.admin.uploadFailed); } finally { setUploading(false); }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      else alert(data.error || t.admin.failedToSave);
    } catch { alert(t.admin.failedToSave); }
  };

  const handleDelete = async (pastTourId: string) => {
    if (!confirm(t.admin.areYouSureDeletePastTour)) return;
    const token = localStorage.getItem("token");
    try {
    ...
    } catch { alert(t.admin.deleteFailed); }
  };
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t.admin.coverImage}</label>
                    ...
                    <label className="w-40 h-28 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center cursor-pointer hover:border-emerald-500 hover:bg-emerald-50 transition-colors">
                      {uploading ? (
                        ...
                        <>
                          ...
                          <span className="text-xs text-gray-500">{t.admin.upload}</span>
                        </>
                      )}
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t.admin.rating}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t.admin.highlightsHint}</label>
