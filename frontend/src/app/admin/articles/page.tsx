...
export default function AdminArticlesPage() {
  ...
  const getCategoryLabel = (category: string) => {
    ...
    return key ? (t.admin[key as keyof typeof t.admin] as string) || category : category;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.error || t.admin.failedToSaveArticle);
      }
    } catch (err) {
  ...
  const handleDelete = async (articleId: string) => {
    if (!confirm(t.admin.areYouSureDeleteArticle)) return;
    const token = localStorage.getItem("token");
    try {
              ...
              <tbody className="divide-y divide-gray-100">
                {articles.map((article) => (
                  <tr key={article.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2 py-1 bg-blue-50 text-blue-700 text-xs rounded font-medium">{getCategoryLabel(article.category)}</span>
                    </td>
          ...
          <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              {activeTab === "en" ? (
                ...
                <>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1" dir="rtl">{t.admin.titleEn}</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1" dir="rtl">{t.admin.descriptionFa}</label>
              ...
              <div>
                ...
                <select name="category" value={formData.category} onChange={handleChange} className="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-emerald-500 text-sm bg-white">
                  {Object.entries(categoryKeys).map(([key]) => (
